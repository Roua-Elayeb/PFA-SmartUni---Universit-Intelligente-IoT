...
const AdminDashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [s, r, p, a] = await Promise.allSettled([
        sensorAPI.getLatest(),
        roomAPI.getAll(),
        parkingAPI.getAll(),
        alertAPI.getAll(),
      ...
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert('✅ Alarme déclenchée', 'Tous les utilisateurs ont été notifiés.');
            fetchData();
          } catch (e) {
      ...
      >
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            <Text style={styles.emptyText}>📡 Aucun capteur connecté</Text>
          </View>
        ) : (
...
const styles = StyleSheet.create({
  ...
  statCard: {
    ...
    marginRight: 12, minWidth: 100, alignItems: 'center',
    borderTopWidth: 3,
  },
  ...
  card: {
    ...
    marginHorizontal: 20, marginBottom: 16, padding: 4,
  },
  sensorRow: {
