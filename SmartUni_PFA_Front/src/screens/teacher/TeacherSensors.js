// src/screens/teacher/TeacherSensors.js
...
const TeacherSensors = () => {
  ...
  useEffect(() => {
    fetchSensors();
    intervalRef.current = setInterval(fetchSensors, 15000);
    return () => clearInterval(intervalRef.current);
  }, [fetchSensors]);

  const filteredSensors = filter === 'all' ? sensors : sensors.filter(s => s.status === filter || s.type === filter);
        ...
        {criticalCount > 0 && (
          <View style={[styles.alertBanner, { borderLeftColor: '#F44336', backgroundColor: '#F4433622' }]}>
            <Text style={[styles.alertBannerText, { color: '#F44336' }]}>
              🚨 {criticalCount} capteur(s) critique(s) — intervention requise
            </Text>
          </View>
        )}
        ...
        {loading ? (
          ...
          <View style={styles.sensorsList}>
            {filteredSensors.map(sensor => {
              const status = sensor.status || getStatusFromValue(sensor.type, sensor.value);
              const sc = STATUS_COLOR[status];
              const sb = STATUS_BG[status];
              const barWidth = getBarWidth(sensor);
              return (
                <View key={sensor._id || sensor.deviceId} style={styles.sensorCard}>
                  ...
                  <View style={styles.barBg}>
                    <View style={[styles.barFill, { width: `${barWidth}%`, backgroundColor: sc }]} />
                  </View>
                  <Text style={styles.timestamp}>
                    🕐 {new Date(sensor.timestamp || sensor.createdAt).toLocaleString('fr-FR', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' })}
                  </Text>
                </View>
              );
            })}
...
const styles = StyleSheet.create({
  ...
  refreshBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: CARD, justifyContent: 'center', alignItems: 'center' },
  ...
  timestamp: { fontSize: 11, color: '#445566' },
